...
use crate::workspace::{config::{find_nearest_project_dir, get_workspace_config}, load::load_projects, query::{find_project_for_dir, find_targets_for_dir, get_all_targets}};
...
pub fn list_command<'a>(input: ListCommandInput<'a>) {
    ...
    let workspace = get_all_targets(projects.values());
    ...
    let mut targets_for_dir = find_targets_for_dir(&workspace, config.workspace_dir.as_path(), project_dir.as_path());

    targets_for_dir.sort();

    for name in targets_for_dir {
